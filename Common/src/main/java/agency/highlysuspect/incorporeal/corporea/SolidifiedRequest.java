...
public final class SolidifiedRequest {
	...
	private final @NotNull CorporeaRequestMatcher matcher;
	private final int count;
	
	private SolidifiedRequest(@NotNull CorporeaRequestMatcher matcher, int count) {
		this.matcher = matcher;
		this.count = count;
	}
	
	public static SolidifiedRequest create(@Nullable CorporeaRequestMatcher matcher, int count) {
		//Try to avoid using vanilla bota's dummy matcher, it doesn't have an ID via CorporeaHelper#registerRequestMatcher.
		CorporeaRequestMatcher matcher2 = matcher == null || matcher == CorporeaRequestMatcher.Dummy.INSTANCE ? EmptyCorporeaRequestMatcher.INSTANCE : matcher;
	...
	public @NotNull CorporeaRequestMatcher matcher() {
		return matcher;
	}
